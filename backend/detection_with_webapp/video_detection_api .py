...
def detect_image():
    ...
    while cap.isOpened():
        ...
        # Convert the raw pixel data (detected data) which is numpy array to JPEG format numpy array data
        _ ,buffer = cv2.imencode('.jpg',frame)

        # Convert the numpy array to bytes data
        frame_bytes = buffer.tobytes()

        # We will not return the frame because we don't want to exit the function.
        # So, use 'yield' is a better option because it can pause after return something and comeback without exiting the function
...
def get_image():
    ...
    return Response(detect_image(),mimetype='multipart/x-mixed-replace; boundary=frame')
