...
# Get the result from detecting License plate
    ...
    if detected_class:
        ...
        print(f"Province Name: {province_name}")

        # Check with database
        check_active_status(license_no)
...
def check_active_status(license_no):
    
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()

        # Get the license_number and status if license_number is existed
        cursor.execute(
            '''SELECT license_number, status FROM registered_cars
               WHERE license_number = ? '''
            ,(license_no,))
        
        # Get only one record from it
        ...
        print(f"License Number: {license_number} with status: {status}")

    except TypeError:
        print("The license number is not found")
